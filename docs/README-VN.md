...
- **`.github/workflows/build.yml`** &mdash; quy trình CI đa nền tảng; các job Windows then chốt là **controller-windows** (bundle Flutter desktop, features mặc định + `hwcodec` + `vram` + `flutter`, không có bridge) và **controlled-windows** (sidecar controlled, `--features vhd-bridge,controlled-only,hwcodec,vram`); cũng chạy script rò rỉ + smoke.
...
2. **CI** &mdash; cùng tên đặt làm repository secret trong GitHub Actions; [`.github/workflows/build.yml`](../.github/workflows/build.yml) tiêm dưới dạng biến môi trường có mặt nạ. **`secret.sec` không bao giờ được hiện thực hóa trên runner**.
